...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.attention_heads = [self.SingleHeadAttention(embedding_dim, head_size) for i in range(num_heads)]
        self.project_layer = nn.Linear(attention_dim, attention_dim, bias=False)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        res = [head(embedded) for head in self.attention_heads]
        res = torch.concat(res, dim=2)
        res = self.project_layer(res)
